...
{
    //用runtime给NSObject关联一个error属性，在解析出错的时候抛出来
...
{
    //返回的是带CoreData的方法，方法中将CoreData置为nil
...
+ (instancetype)mj_objectWithKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    ...
    //这里如果有CoreData对象则对CoreData对象进行处理
...
{
    // 判断是不是模型类，如果自己不是模型类, 那就返回自己
    //一般把字典转模型的任务这里传过来的是个字典，走完这句话会直接将字典返回出去，下面代码将不会执行
...
- (id)mj_JSONObject
{
    //判断是NSString还是NSData类型的数据，并将其转换成json对象
    if ([self isKindOfClass:[NSString class]]) {
